	...
	local function canspam(pl, len)
		...
		if now > nextspam then
			...
			timer.Create("easylua_pspam", 5, 1, function()
				Msg "[Easylua Print] Lost messages due to spam: "
				print(plstr)
			end)
		end
	end
...
function easylua.Start(ply)
	if started then
		Msg "[ELua] "
		print("Session not ended for ", _G.me or (s.vars and s.vars.me), ", restarting session for", ply)
		easylua.End()
	end
	started = true
...
function easylua.End()
	if not started then
		Msg "[ELua] "
		print "Ending session without starting"
	end
	started = false
